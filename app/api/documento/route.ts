...
function buildDiagnostico(
  ...
  if (typeof lineas === "number" && lineas > 0) {
    partes.push(`Líneas en Gestion: ${lineas}. Si en Dynamics hay menos, usa Reprocesar.`);
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    // Cuando hay varios documentos con el mismo Nro_Impreso, priorizar el que tiene líneas en Dynamics (el "correcto")
    const sqlDoc = `
      SELECT TOP 1 Tipo, Numero, Id_Documento, Cod_Empresa, Fecha_Emision, Estado_SII, Lineas_Gestion FROM (
        SELECT 'BLE' AS Tipo, Cab.Nro_Impreso AS Numero, Cab.Id_Boleta AS Id_Documento, Cab.Cod_Empresa, Cab.Fecha_Emision,
               Sii.Estado AS Estado_SII,
               (SELECT COUNT(*) FROM Ges_BlvDetalle D WITH (NOLOCK) WHERE D.Id_Boleta = Cab.Id_Boleta) AS Lineas_Gestion,
        ...
        LEFT JOIN Ges_EleDocSii Sii WITH (NOLOCK) ON Sii.Id_Documento = Cab.Id_Boleta
        WHERE Cab.Nro_Impreso = TRY_CAST(@numero AS INT) OR CONVERT(NVARCHAR(20), Cab.Nro_Impreso) = @numero
        UNION ALL
        SELECT 'FCV', Cab.Nro_Impreso, Cab.Id_Factura, Cab.Cod_Empresa, Cab.Fecha_Emision,
               Sii.Estado,
        ...
        LEFT JOIN Ges_EleDocSii Sii WITH (NOLOCK) ON Sii.Id_Documento = Cab.Id_Factura
        WHERE Cab.Nro_Impreso = TRY_CAST(@numero AS INT) OR CONVERT(NVARCHAR(20), Cab.Nro_Impreso) = @numero
        UNION ALL
        SELECT 'NCV', Cab.Nro_Impreso, Cab.Id_NotaCredito, Cab.Cod_Empresa, Cab.Fecha_Emision,
               Sii.Estado,
        ...
        LEFT JOIN Ges_EleDocSii Sii WITH (NOLOCK) ON Sii.Id_Documento = Cab.Id_NotaCredito
        WHERE Cab.Nro_Impreso = TRY_CAST(@numero AS INT) OR CONVERT(NVARCHAR(20), Cab.Nro_Impreso) = @numero
      ) U
      ORDER BY CntDynamics DESC, Lineas_Gestion DESC
    `;

    const sqlErrores = `
      SELECT Fecha, Mensaje, Tipo, Numero, CONVERT(NVARCHAR(MAX), Error) AS Error
      FROM Ges_Salida_Error_Dyn WITH (NOLOCK)
      WHERE Numero LIKE '%' + @numero + '%' OR CONVERT(NVARCHAR(20), Numero) = @numero
      ORDER BY Fecha DESC
    `;

    const [docs, errores] = await Promise.all([
      query<DocRow[]>(sqlDoc, { numero }),
      query<ErrorRow[]>(sqlErrores, { numero }),
    ]);
