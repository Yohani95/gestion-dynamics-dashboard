  ...
  try {
    ...
    const sqlList = `
      ...
      U AS (
        ...
        LEFT JOIN (SELECT Id_Boleta, COUNT(*) AS Cnt FROM Ges_BlvDetalle WITH (NOLOCK) GROUP BY Id_Boleta) Det ON Det.Id_Boleta = Cab.Id_Boleta
        ...
        UNION ALL
        SELECT 'FCV', Cab.Nro_Impreso, Cab.Id_Factura, Cab.Cod_Empresa, Cab.Fecha_Emision, Sii.Estado, L.Estado, ISNULL(Det.Cnt, 0)
        ...
        WHERE CONVERT(DATE, Cab.Fecha_Emision) = CAST(@fecha AS DATE)
          ...
          AND (NULLIF(RTRIM(@numero), '') IS NULL OR Cab.Nro_Impreso = TRY_CAST(@numero AS INT) OR CONVERT(NVARCHAR(20), Cab.Nro_Impreso) LIKE '%' + @numero + '%')
        UNION ALL
        SELECT 'NCV', Cab.Nro_Impreso, Cab.Id_NotaCredito, Cab.Cod_Empresa, Cab.Fecha_Emision, Sii.Estado, L.Estado, ISNULL(Det.Cnt, 0)
      ...
      U2 AS (
        SELECT Tipo, Numero, Id_Documento, Cod_Empresa, Fecha_Emision, Estado_SII, Estado_Envio, Lineas_Gestion,
               COUNT(*) OVER() AS Total
        FROM U
        WHERE (NULLIF(RTRIM(@tipo), '') IS NULL OR Tipo = RTRIM(@tipo))
      )
      SELECT Tipo, Numero, Id_Documento, Cod_Empresa, Fecha_Emision, Estado_SII, Estado_Envio, Lineas_Gestion, Total
      FROM U2
